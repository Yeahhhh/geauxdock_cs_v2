...
for (int i = 0; i < mcs_nrow; i += bdx_mcs) { // y loop
  float elhm1 = 0.0f;
  int elhm2 = 0;

  const int m = i + tx;
  if (m < mcs_nrow) {
    for (int l = ty; l < lig_natom; l += bdy_mcs) {
      // slower on GK110
      if (CUDA_LDG_D (mcs3->x[l][m]) != MCS_INVALID_COORD OROR1) {
        const float dx = lig_x2[l] - mcs3->x[l][m];
        const float dy = lig_y2[l] - mcs3->y[l][m];
        const float dz = lig_z2[l] - mcs3->z[l][m];
