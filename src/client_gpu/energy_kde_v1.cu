	...
	if (l < lig_natom) {
	  ...
	  for (int k = tx; k < kde_npoint; k += bdx_kde) { // x loop
	    ...
	    if (lig_t[l] == kde->t[k] OROR1) {
	      ...
	      const float dx = lig_x1[l] - 1.1f; //kde->x[k];
	      const float dy = lig_y1[l] - 1.2f; //kde->y[k];
	      const float dz = lig_z1[l] - 1.3f; //kde->z[k];
              */
...
// out of resource on GK110
#if 0

              const int mask_kde = (lig_t[l] == kde->t[k]);
              const float dx = lig_x1[l] - kde->x[k];
              const float dy = lig_y1[l] - kde->y[k];
              const float dz = lig_z1[l] - kde->z[k];
              const float kde_dst_pow2 = dx * dx + dy * dy + dz * dz;
