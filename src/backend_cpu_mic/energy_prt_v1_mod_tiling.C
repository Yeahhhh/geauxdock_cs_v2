...
for (int tile_p1_i = 0; tile_p1_i < prt_npoint; tile_p1_i += TILE_P1) {

    for (int l = 0; l < lig_natom; ++l) {
        float ehpc1 = 0.0f;
        const int lig__t = lig_t[l];

        for (int p = tile_p1_i; p < min(tile_p1_i + TILE_P1, prt_npoint); ++p) {
...
for (int l = 0; l < lig_natom; ++l) {
    const int lig__t = lig_t[l];
    const float hpc2 = (hpc1_s[l] - enepara_hpl0[lig__t]) / enepara_hpl1[lig__t];
